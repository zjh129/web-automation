...
class YdmVerify(object):
    ...
    def __init__(self, **kwargs):
        self._custom_url = kwargs.get("custom_url", self._custom_url)
        self._token = kwargs.get("token", self._token)
    ...
    def common_verify(self, image, verify_type="60000"):
        ...
        if resp_json['code'] != 10000:
            ...
            return self.format_data(resp_json['data']['data'])
    ...
    def click_verify(self, image, label_image=None, extra=None, verify_type="30100", data_type="string"):
        # 通用任意点选1~4个坐标 30009
        ...
        while times < 150:
            try:
                ...
                if result['msg'] == '请求成功' and result['code'] == 10001:
                    ...
                    return self.format_data(result['data']['data'])
    ...
    def format_data(self, data) -> dict | list[dict[str, float]] | str:
        """
        格式化返回数据
        """
        # 判断是否为定位节点列表，但是是字符串形式，此类数据特点是包含|，两类字符串
        if '|' in data and ',' in data:
            ...
            for item in data_list:
                ...
                rs.append({
                    'x': float(item_split[0]),
                    'y': float(item_split[1]),
                })
